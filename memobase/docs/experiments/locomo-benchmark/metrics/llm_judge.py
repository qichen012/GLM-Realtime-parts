...
def evaluate_llm_judge(question, gold_answer, generated_answer, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not response or not response.choices:
                print(f"Attempt {attempt + 1}: Empty response or no choices")
                continue
                
            if len(response.choices) == 0:
                print(f"Attempt {attempt + 1}: No choices available")
                continue
                
            # 检查message和content
            message = response.choices[0].message
            if not message or message.content is None:
                print(f"Attempt {attempt + 1}: No message content")
                continue
            ...
            if "label" not in label_data:
                print(f"Attempt {attempt + 1}: 'label' field not found in response")
                print(f"Response keys: {list(label_data.keys())}")
                continue
            ...
            if label not in ["CORRECT", "INCORRECT"]:
                print(f"Attempt {attempt + 1}: Invalid label value: {label}")
                continue
                
            return 1 if label == "CORRECT" else 0
            
        except Exception as e:
            print(f"Attempt {attempt + 1}: API call failed with error: {e}")
    ...
    # 所有重试都失败后，返回默认值
    print(f"All {max_retries} attempts failed for question: {question[:100]}...")
    return 0
